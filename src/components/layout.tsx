...
export function AppShell({
  ...
  return (
    <div className="flex h-dvh w-full">
      ...
      <div className="flex flex-col flex-1 overflow-hidden pb-[calc(3.75rem+env(safe-area-inset-bottom,0px))] md:pb-0">
        {children}
      </div>
      <MobileNav user={user} />
    </div>
...
export function SplitView({
  ...
  return (
    <div className="flex flex-1 overflow-hidden">
      <div
        style={{ width: `${defaultLeftSize}%`, minWidth: `${minLeftSize}%` }}
        className="flex-shrink-0"
      >
        <ScrollArea className="h-full">{left}</ScrollArea>
      </div>
      <div
        style={{ width: `${100 - defaultLeftSize}%`, minWidth: `${minRightSize}%` }}
        className="flex-1"
      >
        {right}
      </div>
